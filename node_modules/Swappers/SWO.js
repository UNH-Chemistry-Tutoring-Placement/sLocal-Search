...
function SWO() {
    ...
    console.log("****************************************************************************")
    for(s in studentsArr) console.log(studentsArr[s].name)
    for (var i = 0; i<100; i++){
        var assignment = {}
    ...
    for(x in results){
        console.log(results[x].grade)
    }
    console.log("RetRand is: " + retRand)
    return results[0];
}
...
function getTimeForStudent(student, possibleTimes, assignment){
    for(i in student.goodTimes){
        if(isPossible(student.goodTimes[i]) && isOption(student.goodTimes[i])) return student.goodTimes[i]
    }
    for(i in student.possibleTimes){
        if(isPossible(student.possibleTimes[i]) && isOption(student.possibleTimes[i])) return student.possibleTimes[i]
    }

    for(i in student.goodTimes){
        if(isPossible(student.goodTimes[i])) return student.goodTimes[i]
    }

    for(i in student.possibleTimes){
        if(isPossible(student.possibleTimes[i])) return student.possibleTimes[i]
    }
