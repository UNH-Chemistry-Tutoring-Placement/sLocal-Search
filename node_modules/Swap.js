module.exports = Swap;

var grader = require('grader');

function Swap(data, random){
	
	
	//single swap
	for (var time in random){
		
		//grade this time before any changes
		var beforePenalty1 = grader.grade(data, time, random[time]);
		
		for (var student of random[time]){
			
			var afterPenalty1 = grader.gradeWithout(data, time, random[time], student);
			
			//difference
			var diff1 = beforePenalty1 - afterPenalty1;
			
			
			for (var time2 in random){
				if (time2 === time) continue; //ignore same time slot
				
				var beforePenalty2 = grader.grade(data, time2, random[time2]);
				var afterPenalty2 = grader.gradeWith(data, time2, random[time2], student);
				var diff2 = beforePenalty2 - afterPenalty2;
				
				var totalDiff = diff2 + diff1;
				
				if (totalDiff > 0){
					//make the swap
				}
				
			}
		}
	}


}


